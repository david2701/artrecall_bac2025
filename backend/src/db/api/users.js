...
module.exports = class UsersDBApi {
  static async create(data, options) {
    ...
    const users = await db.users.create(
      {
        ...
        last_gps: data.data.last_gps || null,
    ...
    await users.setHobbies(data.data.hobbies || [], {
      transaction,
    });

    await users.setFriends(data.data.friends || [], {
      transaction,
    });
  ...
  static async update(id, data, options) {
    ...
    await users.setHobbies(data.hobbies || [], {
      transaction,
    });

    await users.setFriends(data.friends || [], {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    output.gallery = await users.getGallery({
      transaction,
    });

    output.hobbies = await users.getHobbies({
      transaction,
    });

    output.friends = await users.getFriends({
      transaction,
    });
  ...
  static async findAll(filter, options) {
    ...
    let include = [
...

      {
        model: db.hobbies,
        as: 'hobbies',
        through: filter.hobbies
          ? {
              where: {
                [Op.or]: filter.hobbies.split('|').map((item) => {
                  return { ['Id']: Utils.uuid(item) };
                }),
              },
            }
          : null,
        required: filter.hobbies ? true : null,
      },
    ...
    if (filter) {
      ...
      if (filter.last_gps) {
        where = {
          ...where,
          [Op.and]: Utils.ilike('users', 'last_gps', filter.last_gps),
        };
      }

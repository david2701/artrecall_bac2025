    ...
    <>
      ...
      <SectionMain>
        ...
        <CardBox>
          ...
          <div className={'mb-4'}>
            <p className={'block font-bold mb-2'}>Last Gps</p>
            <p>{users?.last_gps}</p>
          </div>

          <div className={'mb-4'}>
            <p className={'block font-bold mb-2'}>Gallery</p>
            {users?.gallery?.length ? (
              <ImageField
                name={'gallery'}
                image={users?.gallery}
                className='w-20 h-20'
          ...
          <>
            <p className={'block font-bold mb-2'}>Hobbies</p>
            <CardBox
              className='mb-6 border border-gray-300 rounded overflow-hidden'
              hasTable
            >
              <div className='overflow-x-auto'>
                <table>
                  ...
                  <tbody>
                    {users.hobbies &&
                      Array.isArray(users.hobbies) &&
                      users.hobbies.map((item: any) => (
                        <tr
                          key={item.id}
                          onClick={() =>
                            router.push(`/hobbies/hobbies-view/?id=${item.id}`)
                          }
                        >
                          <td data-label='name'>{item.name}</td>
                        </tr>
          ...
          <FormField label='Premium'>
            <SwitchField
              field={{ name: 'premium', value: users?.premium }}
              form={{ setFieldValue: () => null }}
              disabled
            />
          </FormField>
            ...
            >
              <div className='overflow-x-auto'>
                <table>
                  ...
                  <tbody>
                    {users.friends &&
                      Array.isArray(users.friends) &&
                      users.friends.map((item: any) => (
                        <tr
                          key={item.id}
                          onClick={() =>
